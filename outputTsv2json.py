...
@click.command()
@click.option('--tsvfile', multiple=True, help='Original output file generated by a tool.')
@click.option('--prefix', help='Specify prefix input for tools.')

def output2json(tsvfile, prefix):
    """ Simple converter that takes TSV files to generate a summary JSON. """

    df = pd.DataFrame()
    out_dict = {}

    for infile in tsvfile:
        temp = infile.split("/")
        tool = temp[-2]
        idx_col = 'taxRank'
        ...
        if tool == "gottcha2":
            ...
            df['taxRank'] = df['taxRank'].str.replace(r'\bS\b', 'species')
            df['taxRank'] = df['taxRank'].str.replace(r'\bG\b', 'genus')
            df['taxRank'] = df['taxRank'].str.replace(r'\bF\b', 'family')
            result['taxonomyTop10'] = reduceDf(df, ['taxRank', 'name', 'numReads', 'abundance'])
